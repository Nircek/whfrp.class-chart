/*! whfrp.class-chart <=% pkg.version %> Build: <=% grunt.template.today("dd-mm-yyyy") %> */
!function e(t, n, r) {
    function s(o, u) {
        if (!n[o]) {
            if (!t[o]) {
    ...
    1: [ function(require, module, exports) {
        !function() {
            ...
            function d3_selection_each(groups, callback) {
                for (var j = 0, m = groups.length; j < m; j++) for (var node, group = groups[j], i = 0, n = group.length; i < n; i++) (node = group[i]) && callback(node, i, j);
                return groups;
            }
            ...
            function d3_event_dragSuppress(node) {
                ...
                if (null == d3_event_dragSelect && (d3_event_dragSelect = !("onselectstart" in node) && d3_vendorSymbol(node.style, "userSelect")), 
                d3_event_dragSelect) {
                    var style = d3_documentElement(node).style, select = style[d3_event_dragSelect];
                    style[d3_event_dragSelect] = "none";
                }
                return function(suppressClick) {
                    if (w.on(name, null), d3_event_dragSelect && (style[d3_event_dragSelect] = select), 
                    suppressClick) {
                        var off = function() {
                            w.on(click, null);
                        };
            ...
            function d3_hsl_rgb(h, s, l) {
                ...
                var m1, m2;
                return h = isNaN(h) ? 0 : (h %= 360) < 0 ? h + 360 : h, s = isNaN(s) ? 0 : s < 0 ? 0 : s > 1 ? 1 : s, 
                l = l < 0 ? 0 : l > 1 ? 1 : l, m2 = l <= .5 ? l * (1 + s) : l + s - l * s, m1 = 2 * l - m2, 
            ...
            function d3_rgb_parse(format, rgb, hsl) {
                ...
                if (m1 = /([a-z]+)\((.*)\)/.exec(format = format.toLowerCase())) switch (m2 = m1[2].split(","), 
                m1[1]) {
                  case "hsl":
                ...
                r |= r >> 4, g = 240 & color, g |= g >> 4, b = 15 & color, b |= b << 4) : 7 === format.length && (r = (16711680 & color) >> 16, 
            ...
            function d3_rgb_hsl(r, g, b) {
                ...
                return d ? (s = l < .5 ? d / (max + min) : d / (2 - max - min), h = r == max ? (g - b) / d + (g < b ? 6 : 0) : g == max ? (b - r) / d + 2 : (r - g) / d + 4, 
                h *= 60) : (h = NaN, s = l > 0 && l < 1 ? 0 : h), new d3_hsl(h, s, l);
            }
            ...
            function d3_xhr(url, mimeType, response, callback) {
                ...
                return !this.XDomainRequest || "withCredentials" in request || !/^(http(s)?:)?\/\//.test(url) || (request = new XDomainRequest()), 
            ...
            function d3_timer(callback, delay, then) {
                ...
                return d3_timer_queueTail ? d3_timer_queueTail.n = timer : d3_timer_queueHead = timer, 
                d3_timer_queueTail = timer, d3_timer_interval || (d3_timer_timeout = clearTimeout(d3_timer_timeout), 
                d3_timer_interval = 1, d3_timer_frame(d3_timer_step)), timer;
            }
            ...
            function d3_timer_mark() {
                for (var now = Date.now(), timer = d3_timer_queueHead; timer; ) now >= timer.t && timer.c(now - timer.t) && (timer.c = null), 
                timer = timer.n;
                return now;
            }
            function d3_timer_sweep() {
                for (var t0, t1 = d3_timer_queueHead, time = 1 / 0; t1; ) t1.c ? (t1.t < time && (time = t1.t), 
                t1 = (t0 = t1).n) : t1 = t0 ? t0.n = t1.n : d3_timer_queueHead = t1.n;
            ...
            function d3_time_interval(local, step, number) {
                ...
                function range(t0, t1, dt) {
                    ...
                    if (dt > 1) for (;time < t1; ) number(time) % dt || times.push(new Date(+time)), 
                    step(time, 1); else for (;time < t1; ) times.push(new Date(+time)), step(time, 1);
                    return times;
                }
            ...
            function d3_locale_timeFormat(locale) {
                function d3_time_format(template) {
                    ...
                    return format.parse = function(string) {
                        ...
                        return "j" in d ? date.setFullYear(d.y, 0, d.j) : "W" in d || "U" in d ? ("w" in d || (d.w = "W" in d ? 1 : 0), 
                        date.setFullYear(d.y, 0, 1), date.setFullYear(d.y, 0, "W" in d ? (d.w + 6) % 7 + 7 * d.W - (date.getDay() + 5) % 7 : d.w + 7 * d.U - (date.getDay() + 6) % 7)) : date.setFullYear(d.y, d.m, d.d), 
            ...
            function d3_geo_centroidLineStart() {
                ...
                d3_geo_centroid.point = function(\u03bb, \u03c6) {
                    \u03bb *= d3_radians;
                    var cos\u03c6 = Math.cos(\u03c6 *= d3_radians);
                    x0 = cos\u03c6 * Math.cos(\u03bb), y0 = cos\u03c6 * Math.sin(\u03bb), z0 = Math.sin(\u03c6), 
                    d3_geo_centroid.point = nextPoint, d3_geo_centroidPointXYZ(x0, y0, z0);
                };
            }
            ...
            function d3_geo_centroidRingStart() {
                ...
                d3_geo_centroid.point = function(\u03bb, \u03c6) {
                    ...
                    nextPoint(\u03bb00, \u03c600), d3_geo_centroid.lineEnd = d3_geo_centroidLineEnd, 
                    d3_geo_centroid.point = d3_geo_centroidPoint;
                };
            }
                    ...
                    for (var points, point, start = subject[0]; ;) {
                        ...
                        do {
                            if (current.v = current.o.v = !0, current.e) {
                                if (isSubject) for (var i = 0, n = points.length; i < n; ++i) listener.point((point = points[i])[0], point[1]); else interpolate(current.x, current.n.x, 1, listener);
                                current = current.n;
                            } else {
                                if (isSubject) {
                                    points = current.p.z;
            ...
            function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
                var \u03c6;
                if (null == from) \u03c6 = direction * half\u03c0, listener.point(-\u03c0, \u03c6), 
                listener.point(0, \u03c6), listener.point(\u03c0, \u03c6), listener.point(\u03c0, 0), 
                listener.point(\u03c0, -\u03c6), listener.point(0, -\u03c6), listener.point(-\u03c0, -\u03c6), 
                listener.point(-\u03c0, 0), listener.point(-\u03c0, \u03c6); else if (abs(from[0] - to[0]) > \u03b5) {
                    var s = from[0] < to[0] ? \u03c0 : -\u03c0;
                    \u03c6 = direction * s / 2, listener.point(-s, \u03c6), listener.point(0, \u03c6), 
                    listener.point(s, \u03c6);
            ...
            function d3_geo_clipCircle(radius) {
                ...
                function clipLine(listener) {
                    ...
                    return {
                        ...
                        point: function(\u03bb, \u03c6) {
                            var point2, point1 = [ \u03bb, \u03c6 ], v = visible(\u03bb, \u03c6), c = smallRadius ? v ? 0 : code(\u03bb, \u03c6) : v ? code(\u03bb + (\u03bb < 0 ? \u03c0 : -\u03c0), \u03c6) : 0;
                ...
                function intersect(a, b, two) {
                    ...
                    var c1 = cr * n2n2 / determinant, c2 = -cr * n1n2 / determinant, n1xn2 = d3_geo_cartesianCross(n1, n2), A = d3_geo_cartesianScale(n1, c1);
                ...
                function code(\u03bb, \u03c6) {
                    ...
                    return \u03bb < -r ? code |= 1 : \u03bb > r && (code |= 2), \u03c6 < -r ? code |= 4 : \u03c6 > r && (code |= 8), 
                    code;
                }
            ...
            function d3_geo_clipExtent(x0, y0, x1, y1) {
                ...
                return function(listener) {
                    function insidePolygon(p) {
                        for (var wn = 0, n = polygon.length, y = p[1], i = 0; i < n; ++i) for (var b, j = 1, v = polygon[i], m = v.length, a = v[0]; j < m; ++j) b = v[j], 
                    ...
                    function interpolate(from, to, direction, listener) {
                        var a = 0, a1 = 0;
                        if (null == from || (a = corner(from, direction)) !== (a1 = corner(to, direction)) || comparePoints(from, to) < 0 ^ direction > 0) do {
                            listener.point(0 === a || 3 === a ? x0 : x1, a > 1 ? y1 : y0);
                        } while ((a = (a + direction + 4) % 4) !== a1); else listener.point(to[0], to[1]);
                    }
            ...
            function d3_geo_pathBoundsPoint(x, y) {
                x < d3_geo_pathBoundsX0 && (d3_geo_pathBoundsX0 = x), x > d3_geo_pathBoundsX1 && (d3_geo_pathBoundsX1 = x), 
                y < d3_geo_pathBoundsY0 && (d3_geo_pathBoundsY0 = y), y > d3_geo_pathBoundsY1 && (d3_geo_pathBoundsY1 = y);
            }
            ...
            function d3_geo_resample(project) {
                ...
                function resampleRecursive(stream) {
                    ...
                    function ringPoint(\u03bb, \u03c6) {
                        ...
                        c00 = c0, resample.point = linePoint;
                    }
                    function ringEnd() {
                ...
                function resampleLineTo(x0, y0, \u03bb0, a0, b0, c0, x1, y1, \u03bb1, a1, b1, c1, depth, stream) {
                    ...
                    if (d2 > 4 * \u03b42 && depth--) {
                        ...
                        (dz * dz / d2 > \u03b42 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) && (resampleLineTo(x0, y0, \u03bb0, a0, b0, c0, x2, y2, \u03bb2, a /= m, b /= m, c, depth, stream), 
            ...
            function d3_geo_circleInterpolate(radius, precision) {
                ...
                return function(from, to, direction, listener) {
                    ...
                    (direction > 0 ? from < to : from > to) && (from += direction * \u03c4)) : (from = radius + direction * \u03c4, 
                    to = radius - .5 * step);
            ...
            function d3_geom_voronoiAddBeach(site) {
                for (var lArc, rArc, dxl, dxr, x = site.x, directrix = site.y, node = d3_geom_voronoiBeaches._; node; ) if ((dxl = d3_geom_voronoiLeftBreakPoint(node, directrix) - x) > \u03b5) node = node.L; else {
            ...
            function d3_geom_voronoiCloseCells(extent) {
                for (var x2, y2, x3, y3, cell, iHalfEdge, halfEdges, nHalfEdges, start, end, x0 = extent[0][0], x1 = extent[1][0], y0 = extent[0][1], y1 = extent[1][1], cells = d3_geom_voronoiCells, iCell = cells.length; iCell--; ) if ((cell = cells[iCell]) && cell.prepare()) for (halfEdges = cell.edges, 
                nHalfEdges = halfEdges.length, iHalfEdge = 0; iHalfEdge < nHalfEdges; ) end = halfEdges[iHalfEdge].end(), 
                x3 = end.x, y3 = end.y, start = halfEdges[++iHalfEdge % nHalfEdges].start(), x2 = start.x, 
                y2 = start.y, (abs(x3 - x2) > \u03b5 || abs(y3 - y2) > \u03b5) && (halfEdges.splice(iHalfEdge, 0, new d3_geom_voronoiHalfEdge(d3_geom_voronoiCreateBorderEdge(cell.site, end, abs(x3 - x0) < \u03b5 && y1 - y3 > \u03b5 ? {
                    x: x0,
                    y: abs(x2 - x0) < \u03b5 ? y2 : y1
                } : abs(y3 - y1) < \u03b5 && x1 - x3 > \u03b5 ? {
                    x: abs(y2 - y1) < \u03b5 ? x2 : x1,
            ...
            function d3_geom_quadtreeFind(root, x, y, x0, y0, x3, y3) {
                ...
                return function find(node, x1, y1, x2, y2) {
                    if (!(x1 > x3 || y1 > y3 || x2 < x0 || y2 < y0)) {
                        ...
                        for (var children = node.nodes, xm = .5 * (x1 + x2), ym = .5 * (y1 + y2), right = x >= xm, below = y >= ym, i = below << 1 | right, j = i + 4; i < j; ++i) if (node = children[3 & i]) switch (3 & i) {
                          case 0:
            ...
            function d3_interpolateTransform(a, b) {
                var s = [], q = [];
                return a = d3.transform(a), b = d3.transform(b), d3_interpolateTranslate(a.translate, b.translate, s, q), 
                d3_interpolateRotate(a.rotate, b.rotate, s, q), d3_interpolateSkew(a.skew, b.skew, s, q), 
                d3_interpolateScale(a.scale, b.scale, s, q), a = b = null, function(t) {
            ...
            function d3_layout_forceAccumulate(quad, alpha, charges) {
                var cx = 0, cy = 0;
                if (quad.charge = 0, !quad.leaf) for (var c, nodes = quad.nodes, n = nodes.length, i = -1; ++i < n; ) null != (c = nodes[i]) && (d3_layout_forceAccumulate(c, alpha, charges), 
                quad.charge += c.charge, cx += c.charge * c.cx, cy += c.charge * c.cy);
                if (quad.point) {
            ...
            function d3_layout_stackMaxIndex(array) {
                for (var k, i = 1, j = 0, v = array[0][1], n = array.length; i < n; ++i) (k = array[i][1]) > v && (j = i, 
                v = k);
                return j;
            }
            ...
            function d3_layout_packSiblings(node) {
                ...
                if ((nodes = node.children) && (n = nodes.length)) {
                    ...
                    d3_layout_packInsert(c, b), b = a._pack_next, i = 3; i < n; i++) {
                        ...
                        s2++) ;
                        isect ? (s1 < s2 || s1 == s2 && b.r < a.r ? d3_layout_packSplice(a, b = j) : d3_layout_packSplice(a = k, b), 
            ...
            function d3_scale_nice(domain, nice) {
                ...
                return x1 < x0 && (dx = i0, i0 = i1, i1 = dx, dx = x0, x0 = x1, x1 = dx), domain[i0] = nice.floor(x0), 
            ...
            function d3_scale_linearTickRange(domain, m) {
                ...
                return err <= .15 ? step *= 10 : err <= .35 ? step *= 5 : err <= .75 && (step *= 2), 
            ...
            function d3_scale_ordinal(domain, ranger) {
                ...
                return scale.domain = function(x) {
                    ...
                    return range = steps(start + Math.round(step * padding / 2 + (stop - start - (domain.length - 1 + padding) * step) / 2), step), 
                    rangeBand = 0, ranger = {
                        t: "rangeRoundPoints",
                        a: arguments
                    }, scale;
                    ...
                    return range = steps(start + Math.round((stop - start - (domain.length - padding) * step) / 2), step), 
                    reverse && range.reverse(), rangeBand = Math.round(step * (1 - padding)), ranger = {
                        t: "rangeRoundBands",
                        a: arguments
                    }, scale;
            ...
            function d3_svg_arcCornerTangents(p0, p1, r1, rc, cw) {
                ...
                [ [ cx0 - ox, cy0 - oy ], [ cx0 * r1 / r, cy0 * r1 / r ] ];
            }
            ...
            function d3_svg_lineMonotoneTangents(points) {
                ...
                abs(d) < \u03b5 ? m[i] = m[i + 1] = 0 : (a = m[i] / d, b = m[i + 1] / d, (s = a * a + b * b) > 9 && (s = 3 * d / Math.sqrt(s), 
                m[i] = s * a, m[i + 1] = s * b));
            ...
            function d3_svg_lineRadial(points) {
                ...
                a = point[1] - half\u03c0, point[0] = r * Math.cos(a), point[1] = r * Math.sin(a);
                return points;
            }
            ...
            function d3_selection_interruptNS(ns) {
                return function() {
                    ...
                    (lock = this[ns]) && (active = lock[activeId = lock.active]) && (active.timer.c = null, 
                    active.timer.t = NaN, --lock.count ? delete lock[activeId] : delete this[ns], lock.active += .5, 
                    active.event && active.event.interrupt.call(this, this.__data__, active.index));
                };
            }
            function d3_transition(groups, ns, id) {
                return d3_subclass(groups, d3_transitionPrototype), groups.namespace = ns, groups.id = id, 
                groups;
            }
            ...
            function d3_transitionNode(node, i, ns, id, inherit) {
                ...
                function start(elapsed) {
                    ...
                    active && (active.timer.c = null, active.timer.t = NaN, --lock.count, delete lock[activeId], 
                    ...
                    }, 0, time), lock.active = id, transition.event && transition.event.start.call(node, node.__data__, i), 
                    tweens = [], transition.tween.forEach(function(key, value) {
                ...
                function tick(elapsed) {
                    ...
                    if (t >= 1) return transition.event && transition.event.end.call(node, node.__data__, i), 
                    --lock.count ? delete lock[id] : delete node[ns], 1;
                }
            ...
            d3.ascending = d3_ascending, d3.descending = function(a, b) {
                ...
                if (1 === arguments.length) for (;++i < n; ) d3_numeric(a = d3_number(array[i])) && (d = a - m, 
                m += d / ++j, s += d * (a - m)); else for (;++i < n; ) d3_numeric(a = d3_number(f.call(array, array[i], i))) && (d = a - m, 
                m += d / ++j, s += d * (a - m));
                if (j > 1) return s / (j - 1);
            ...
            d3.bisectLeft = d3_bisect.left, d3.bisect = d3.bisectRight = d3_bisect.right, d3.bisector = function(f) {
                ...
                for (var t, i, m = i1 - i0; m; ) i = Math.random() * m-- | 0, t = array[m + i0], 
                ...
                for (var i = -1, m = d3.min(matrix, d3_transposeLength), transpose = new Array(m); ++i < m; ) for (var n, j = -1, row = transpose[i] = new Array(n); ++j < n; ) row[j] = matrix[j][i];
                return transpose;
            }, d3.zip = function() {
            ...
            d3.range = function(start, stop, step) {
                ...
                if (start *= k, stop *= k, step *= k, step < 0) for (;(j = start + step * ++i) > stop; ) range.push(j / k); else for (;(j = start + step * ++i) < stop; ) range.push(j / k);
                return range;
            ...
            var d3_nsXhtml = "http://www.w3.org/1999/xhtml", d3_nsPrefix = {
            ...
            d3.ns = {
                ...
                function bind(group, groupData) {
                    ...
                    if (key) {
                        ...
                        for (i = -1; ++i < n; ) (node = group[i]) && (nodeByKeyValue.has(keyValue = key.call(node, node.__data__, i)) ? exitNodes[i] = node : nodeByKeyValue.set(keyValue, node), 
                        keyValues[i] = keyValue);
                        for (i = -1; ++i < m; ) (node = nodeByKeyValue.get(keyValue = key.call(groupData, nodeData = groupData[i], i))) ? !0 !== node && (updateNodes[i] = node, 
            ...
            d3_selection_enterPrototype.size = d3_selectionPrototype.size, d3_selection_enterPrototype.select = function(selector) {
                ...
                var group;
                return "string" == typeof node ? (group = [ d3_select(node, d3_document) ], group.parentNode = d3_document.documentElement) : (group = [ node ], 
                group.parentNode = d3_documentElement(node)), d3_selection([ group ]);
            }, d3.selectAll = function(nodes) {
                var group;
                return "string" == typeof nodes ? (group = d3_array(d3_selectAll(nodes, d3_document)), 
                group.parentNode = d3_document.documentElement) : (group = d3_array(nodes), group.parentNode = null), 
                d3_selection([ group ]);
            ...
            d3.touch = function(container, touches, identifier) {
                ...
                touches) for (var touch, i = 0, n = touches.length; i < n; ++i) if ((touch = touches[i]).identifier === identifier) return d3_mousePoint(container, touch);
            ...
            d3.interpolateZoom = function(p0, p1) {
                ...
                function zoomTo(that, p, l, k) {
                    that.__chart__ = {
                        ...
                        y: view.y,
                        k: view.k
                    }, scaleTo(Math.pow(2, k)), translateTo(center0 = p, l), that = d3.select(that), 
                    duration > 0 && (that = that.transition().duration(duration)), that.call(zoom.event);
                }
                function rescale() {
                ...
                function touchstarted() {
                    ...
                    function moved() {
                        ...
                        for (var i = 0, n = touches.length; i < n; ++i, l1 = null) if (p1 = touches[i], 
                    ...
                    var scale0, that = this, dispatch = event.of(that, arguments), locations0 = {}, distance0 = 0, zoomName = ".zoom-" + d3.event.changedTouches[0].identifier, touchmove = "touchmove" + zoomName, touchend = "touchend" + zoomName, targets = [], subject = d3.select(that), dragRestore = d3_event_dragSuppress(that);
            ...
            d3_rgbPrototype.brighter = function(k) {
                ...
                return r || g || b ? (r && r < i && (r = i), g && g < i && (g = i), b && b < i && (b = i), 
            ...
            d3.formatPrefix = function(value, precision) {
                var i = 0;
                return (value = +value) && (value < 0 && (value *= -1), precision && (value = d3.round(value, d3_format_precision(value, precision))), 
            ...
            d3.geo.bounds = function() {
                function point(\u03bb, \u03c6) {
                    ranges.push(range = [ \u03bb0 = \u03bb, \u03bb1 = \u03bb ]), \u03c6 < \u03c60 && (\u03c60 = \u03c6), 
                    \u03c6 > \u03c61 && (\u03c61 = \u03c6);
                }
                function linePoint(\u03bb, \u03c6) {
                    ...
                    if (p0) {
                        ...
                        antimeridian ? \u03bb < \u03bb_ ? angle(\u03bb0, \u03bb) > angle(\u03bb0, \u03bb1) && (\u03bb1 = \u03bb) : angle(\u03bb, \u03bb1) > angle(\u03bb0, \u03bb1) && (\u03bb0 = \u03bb) : \u03bb1 >= \u03bb0 ? (\u03bb < \u03bb0 && (\u03bb0 = \u03bb), 
                ...
                var \u03bb0, \u03c60, \u03bb1, \u03c61, \u03bb_, \u03bb__, \u03c6__, p0, d\u03bbSum, ranges, range, bound = {
                    ...
                    polygonEnd: function() {
                        ...
                        d3_geo_areaRingSum < 0 ? (\u03bb0 = -(\u03bb1 = 180), \u03c60 = -(\u03c61 = 90)) : d\u03bbSum > \u03b5 ? \u03c61 = 90 : d\u03bbSum < -\u03b5 && (\u03c60 = -90), 
                ...
                return function(feature) {
                    ...
                    if (n) {
                        ...
                        for (var b, i = 1, a = ranges[0], merged = [ a ]; i < n; ++i) b = ranges[i], withinRange(b[0], a) || withinRange(b[1], a) ? (angle(a[0], b[1]) > angle(a[0], a[1]) && (a[1] = b[1]), 
                        angle(b[0], a[1]) > angle(a[0], a[1]) && (a[0] = b[0])) : merged.push(a = b);
                        for (var d\u03bb, b, best = -1 / 0, n = merged.length - 1, i = 0, a = merged[n]; i <= n; a = b, 
                        ++i) b = merged[i], (d\u03bb = angle(a[1], b[0])) > best && (best = d\u03bb, \u03bb0 = b[0], 
                        \u03bb1 = a[1]);
                    }
                ...
                d3_geo_centroidW1 < \u03b5 && (x = d3_geo_centroidX0, y = d3_geo_centroidY0, z = d3_geo_centroidZ0), 
                (m = x * x + y * y + z * z) < \u03b52) ? [ NaN, NaN ] : [ Math.atan2(y, x) * d3_degrees, d3_asin(z / Math.sqrt(m)) * d3_degrees ];
            };
            ...
            d3_geom_voronoiCell.prototype.prepare = function() {
                ...
                function voronoi(data) {
                    ...
                    return d3_geom_voronoi(sites(data), clipExtent).cells.forEach(function(cell, i) {
                        ...
                        (polygons[i] = edges.length ? edges.map(function(e) {
                            var s = e.start();
                            return [ s.x, s.y ];
                        }) : site.x >= x0 && site.x <= x1 && site.y >= y0 && site.y <= y1 ? [ [ x0, y1 ], [ x1, y1 ], [ x1, y0 ], [ x0, y0 ] ] : []).point = data[i];
                    }), polygons;
                }
                function sites(data) {
                ...
                return points ? voronoi(points) : (voronoi.links = function(data) {
                    ...
                    return d3_geom_voronoi(sites(data)).cells.forEach(function(cell, i) {
                        for (var s0, site = cell.site, edges = cell.edges.sort(d3_geom_voronoiHalfEdgeOrder), j = -1, m = edges.length, e1 = edges[m - 1].edge, s1 = e1.l === site ? e1.r : e1.l; ++j < m; ) e1, 
            ...
            d3.geom.delaunay = function(vertices) {
                ...
                function quadtree(data) {
                    ...
                    function insertChild(n, d, x, y, x1, y1, x2, y2) {
                        ...
                        n.leaf = !1, n = n.nodes[i] || (n.nodes[i] = d3_geom_quadtreeNode()), right ? x1 = xm : x2 = xm, 
                        below ? y1 = ym : y2 = ym, insert(n, d, x, y, x1, y1, x2, y2);
                    }
                    ...
                    xs = [], ys = [], n = data.length, compat) for (i = 0; i < n; ++i) d = data[i], 
                    d.x < x1_ && (x1_ = d.x), d.y < y1_ && (y1_ = d.y), d.x > x2_ && (x2_ = d.x), d.y > y2_ && (y2_ = d.y), 
                    xs.push(d.x), ys.push(d.y); else for (i = 0; i < n; ++i) {
                        var x_ = +fx(d = data[i], i), y_ = +fy(d, i);
                        x_ < x1_ && (x1_ = x_), y_ < y1_ && (y1_ = y_), x_ > x2_ && (x2_ = x_), y_ > y2_ && (y2_ = y_), 
            ...
            d3.interpolateTransform = d3_interpolateTransform, d3.layout = {}, d3.layout.bundle = function() {
                ...
                var timer, drag, alpha, distances, strengths, charges, force = {}, event = d3.dispatch("start", "tick", "end"), size = [ 1, 1 ], friction = .9, linkDistance = d3_layout_forceLinkDistance, linkStrength = d3_layout_forceLinkStrength, charge = -30, chargeDistance2 = d3_layout_forceChargeDistance2, gravity = .1, theta2 = .64, nodes = [], links = [];
                return force.tick = function() {
                    ...
                    for (i = 0; i < m; ++i) o = links[i], s = o.source, t = o.target, x = t.x - s.x, 
                    y = t.y - s.y, (l = x * x + y * y) && (l = alpha * strengths[i] * ((l = Math.sqrt(l)) - distances[i]) / l, 
                    x *= l, y *= l, t.x -= x * (k = s.weight + t.weight ? s.weight / (s.weight + t.weight) : .5), 
                    t.y -= y * k, s.x += x * (k = 1 - k), s.y += y * k);
                    ...
                    return arguments.length ? (x = +x, alpha ? x > 0 ? alpha = x : (timer.c = null, 
                    timer.t = NaN, timer = null, event.end({
                        type: "end",
                        alpha: alpha = 0
                    })) : x > 0 && (event.start({
                        type: "start",
                    ...
                    function position(dimension, size) {
                        ...
                        for (var x, candidates = neighbors[i], j = -1, l = candidates.length; ++j < l; ) if (!isNaN(x = candidates[j][dimension])) return x;
                        return Math.random() * size;
                    }
                    var i, neighbors, o, n = nodes.length, m = links.length, w = size[0], h = size[1];
                    for (i = 0; i < n; ++i) (o = nodes[i]).index = i, o.weight = 0;
                    for (i = 0; i < m; ++i) o = links[i], "number" == typeof o.source && (o.source = nodes[o.source]), 
                    "number" == typeof o.target && (o.target = nodes[o.target]), ++o.source.weight, 
                    ++o.target.weight;
                    for (i = 0; i < n; ++i) o = nodes[i], isNaN(o.x) && (o.x = position("x", w)), isNaN(o.y) && (o.y = position("y", h)), 
                    isNaN(o.px) && (o.px = o.x), isNaN(o.py) && (o.py = o.y);
                    if (distances = [], "function" == typeof linkDistance) for (i = 0; i < m; ++i) distances[i] = +linkDistance.call(this, links[i], i); else for (i = 0; i < m; ++i) distances[i] = linkDistance;
                    if (strengths = [], "function" == typeof linkStrength) for (i = 0; i < m; ++i) strengths[i] = +linkStrength.call(this, links[i], i); else for (i = 0; i < m; ++i) strengths[i] = linkStrength;
                    if (charges = [], "function" == typeof charge) for (i = 0; i < n; ++i) charges[i] = +charge.call(this, nodes[i], i); else for (i = 0; i < n; ++i) charges[i] = charge;
                    return force.resume();
            ...
            d3.layout.hierarchy = function() {
                ...
                function pie(data) {
                    ...
                    }), a = +("function" == typeof startAngle ? startAngle.apply(this, arguments) : startAngle), da = ("function" == typeof endAngle ? endAngle.apply(this, arguments) : endAngle) - a, p = Math.min(Math.abs(da) / n, +("function" == typeof padAngle ? padAngle.apply(this, arguments) : padAngle)), pa = p * (da < 0 ? -1 : 1), sum = d3.sum(values), k = sum ? (da - n * pa) / sum : 0, index = d3.range(n), arcs = [];
            ...
            d3.layout.stack = function() {
                function stack(data, index) {
                    ...
                    for (j = 0; j < m; ++j) for (out.call(stack, series[0][j], o = offsets[j], points[0][j][1]), 
                    i = 1; i < n; ++i) out.call(stack, series[i][j], o += points[i - 1][j][1], points[i][j][1]);
                    return data;
                }
            ...
            var d3_layout_stackOrders = d3.map({
                "inside-out": function(data) {
                    ...
                    for (i = 0; i < n; ++i) j = index[i], top < bottom ? (top += sums[j], tops.push(j)) : (bottom += sums[j], 
                    bottoms.push(j));
            ...
            d3.layout.histogram = function() {
                function histogram(data, i) {
                    ...
                    if (m > 0) for (i = -1; ++i < n; ) (x = values[i]) >= range[0] && x <= range[1] && (bin = bins[d3.bisect(thresholds, x, 1, m) - 1], 
                ...
                function wrapTree(root0) {
                    for (var node1, root1 = {
                        A: null,
                        children: [ root0 ]
                    }, queue = [ root1 ]; null != (node1 = queue.pop()); ) for (var child, children = node1.children, i = 0, n = children.length; i < n; ++i) queue.push((children[i] = child = {
                        _: children[i],
                        parent: node1,
                ...
                function scale(children, k) {
                    for (var child, area, i = -1, n = children.length; ++i < n; ) area = (child = children[i]).value * (k < 0 ? 0 : k), 
                ...
                function worst(row, u) {
                    for (var r, s = row.area, rmax = 0, rmin = 1 / 0, i = -1, n = row.length; ++i < n; ) (r = row[i].area) && (r < rmin && (rmin = r), 
                    r > rmax && (rmax = r));
                ...
                function treemap(d) {
                    ...
                    return root.x = root.y = 0, root.value ? (root.dx = size[0], root.dy = size[1]) : root.dx = root.dy = 0, 
                    stickies && hierarchy.revalue(root), scale([ root ], root.dx * root.dy / root.value), 
                    (stickies ? stickify : squarify)(root), sticky && (stickies = nodes), nodes;
                }
                ...
                normal: function(\u00b5, \u03c3) {
                    var n = arguments.length;
                    return n < 2 && (\u03c3 = 1), n < 1 && (\u00b5 = 0), function() {
                        var x, y, r;
                        do {
            ...
            d3.scale.quantile = function() {
                ...
                function arc() {
                    ...
                    if ((ap = (+padAngle.apply(this, arguments) || 0) / 2) && (rp = padRadius === d3_svg_arcAuto ? Math.sqrt(r0 * r0 + r1 * r1) : +padRadius.apply(this, arguments), 
                    cw || (p1 *= -1), r1 && (p1 = d3_asin(rp / r1 * Math.sin(ap))), r0 && (p0 = d3_asin(rp / r0 * Math.sin(ap)))), 
                    r1) {
                        x0 = r1 * Math.cos(a0 + p1), y0 = r1 * Math.sin(a0 + p1), x1 = r1 * Math.cos(a1 - p1), 
                    ...
                    if (r0) {
                        x2 = r0 * Math.cos(a1 - p0), y2 = r0 * Math.sin(a1 - p0), x3 = r0 * Math.cos(a0 + p0), 
                    ...
                    if (da > \u03b5 && (rc = Math.min(Math.abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments))) > .001) {
                        ...
                        if (null != x1) {
                            var t30 = d3_svg_arcCornerTangents(null == x3 ? [ x2, y2 ] : [ x3, y3 ], [ x0, y0 ], r1, rc1, cw), t12 = d3_svg_arcCornerTangents([ x1, y1 ], [ x2, y2 ], r1, rc1, cw);
                        ...
                        if (null != x3) {
                            var t03 = d3_svg_arcCornerTangents([ x0, y0 ], [ x3, y3 ], r0, -rc0, cw), t21 = d3_svg_arcCornerTangents([ x2, y2 ], null == x1 ? [ x0, y0 ] : [ x1, y1 ], r0, -rc0, cw);
                    ...
                    } else path.push("M", x0, ",", y0), null != x1 && path.push("A", r1, ",", r1, " 0 ", l1, ",", cw, " ", x1, ",", y1), 
                    path.push("L", x2, ",", y2), null != x3 && path.push("A", r0, ",", r0, " 0 ", l0, ",", 1 - cw, " ", x3, ",", y3);
                ...
                return arc.innerRadius = function(v) {
                    ...
                    return arguments.length ? (padRadius = v == d3_svg_arcAuto ? d3_svg_arcAuto : d3_functor(v), 
                    arc) : padRadius;
            ...
            d3_selectionPrototype.transition = function(name) {
                for (var subgroup, node, id = d3_transitionInheritId || ++d3_transitionId, ns = d3_transitionNamespace(name), subgroups = [], transition = d3_transitionInherit || {
                    ...
                    for (var group = this[j], i = -1, n = group.length; ++i < n; ) (node = group[i]) && d3_transitionNode(node, i, ns, id, transition), 
                    subgroup.push(node);
                }
            ...
            d3.transition = function(selection, name) {
                ...
                for (var j = -1, m = this.length; ++j < m; ) {
                    ...
                    d3_transitionNode(subnode, i, ns, id, node[ns][id]), subgroup.push(subnode)) : subgroup.push(null);
                }
                return d3_transition(subgroups, ns, id);
                ...
                for (var j = -1, m = this.length; ++j < m; ) for (var group = this[j], i = -1, n = group.length; ++i < n; ) if (node = group[i]) {
                    transition = node[ns][id], subnodes = selector.call(node, node.__data__, i, j), 
                    subgroups.push(subgroup = []);
                    for (var k = -1, o = subnodes.length; ++k < o; ) (subnode = subnodes[k]) && d3_transitionNode(subnode, k, ns, id, transition), 
                ...
                var id = this.id, ns = this.namespace;
                return arguments.length < 1 ? this.node()[ns][id].ease : ("function" != typeof value && (value = d3.ease.apply(d3, arguments)), 
                ...
                for (var subgroup, group, node, transition, id0 = this.id, id1 = ++d3_transitionId, ns = this.namespace, subgroups = [], j = 0, m = this.length; j < m; j++) {
                    subgroups.push(subgroup = []);
                    for (var group = this[j], i = 0, n = group.length; i < n; i++) (node = group[i]) && (transition = node[ns][id0], 
                ...
                function axis(g) {
                    g.each(function() {
                        ...
                        y1 = "y", x2 = "x2", y2 = "y2", text.attr("dy", sign < 0 ? "0em" : ".71em").style("text-anchor", "middle"), 
                        pathUpdate.attr("d", "M" + range[0] + "," + sign * outerTickSize + "V0H" + range[1] + "V" + sign * outerTickSize)) : (tickTransform = d3_svg_axisY, 
                        x1 = "y", y1 = "x", x2 = "y2", y2 = "x2", text.attr("dy", ".32em").style("text-anchor", sign < 0 ? "end" : "start"), 
            ...
            d3.svg.brush = function() {
                ...
                function brushstart() {
                    ...
                    function move1(point, scale, i) {
                        ...
                        if (dragging && (r0 -= position, r1 -= size + position), min = (i ? yClamp : xClamp) ? Math.max(r0, Math.min(r1, point[i])) : point[i], 
                    ...
                    var center, offset, target = this, eventTarget = d3.select(d3.event.target), event_ = event.of(target, arguments), g = d3.select(target), resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x, resizingY = !/^(e|w)$/.test(resizing) && y, dragging = eventTarget.classed("extent"), dragRestore = d3_event_dragSuppress(target), origin = d3.mouse(target), w = d3.select(d3_window(target)).on("keydown.brush", keydown).on("keyup.brush", keyup);
                ...
                return brush.event = function(g) {
                    ...
                    xExtentDomain = [ x0, x1 ], x.invert && (x0 = x(x0), x1 = x(x1)), x1 < x0 && (t = x0, 
                    x0 = x1, x1 = t), x0 == xExtent[0] && x1 == xExtent[1] || (xExtent = [ x0, x1 ])), 
                    y && (y0 = z[0], y1 = z[1], x && (y0 = y0[1], y1 = y1[1]), yExtentDomain = [ y0, y1 ], 
                    y.invert && (y0 = y(y0), y1 = y(y1)), y1 < y0 && (t = y0, y0 = y1, y1 = t), y0 == yExtent[0] && y1 == yExtent[1] || (yExtent = [ y0, y1 ])), 
                    brush) : (x && (xExtentDomain ? (x0 = xExtentDomain[0], x1 = xExtentDomain[1]) : (x0 = xExtent[0], 
                    x1 = xExtent[1], x.invert && (x0 = x.invert(x0), x1 = x.invert(x1)), x1 < x0 && (t = x0, 
                    x0 = x1, x1 = t))), y && (yExtentDomain ? (y0 = yExtentDomain[0], y1 = yExtentDomain[1]) : (y0 = yExtent[0], 
                    y1 = yExtent[1], y.invert && (y0 = y.invert(y0), y1 = y.invert(y1)), y1 < y0 && (t = y0, 
    ...
    2: [ function(require, module, exports) {
        function IDOrName(id) {
            return careers[id] ? careers[id].name : id;
        }
        function setActive(career_name) {
            careers[career_name] ? (d3.select("#list_" + active_career).classed("active", !1), 
            d3.select("#wheel_" + active_career).classed("active", !1), active_career = career_name, 
            d3.select("#list_" + active_career).classed("active", !0), d3.select("#wheel_" + active_career).classed("active", !0), 
            showTooltip(career_name), highlightWheel(career_name)) : console.log("Sorry! No record in db for " + career_name);
        }
        function highlightWheel(d) {
            link.classed("entrie exit", !1).classed("exit", function(i) {
                return i.source.id == d;
            }).classed("entrie", function(i) {
                return i.target.id == d;
            }), node.classed("entrie exit", !1).classed("entrie", function(i) {
                return careers[d].entries.indexOf(i.id) > -1;
            }).classed("exit", function(i) {
                return careers[d].exits.indexOf(i.id) > -1;
            });
        }
        function mouseOver(d) {
            showTooltip(d.id), highlightWheel(d.id);
        }
        function mouseOut(d) {
            showTooltip(active_career), highlightWheel(active_career);
        }
        function showTooltip(career_name) {
            career = careers[career_name], d3.select("#right-pannel-about").classed("hidden", void 0 != career), 
            d3.select("#right-pannel-info").classed("hidden", void 0 == career), career && (d3.select("#classname").text(career.name), 
            d3.select("#quote").text(career.quote), d3.select("#skills").text(career.skills), 
            d3.select("#talents").text(career.talents), d3.select("#trappings").text(career.trappings), 
            d3.select("#description").text(career.description), d3.select("#main-profile").selectAll("td").data(career.mainprofile).text(function(d) {
                return d;
            }), d3.select("#secondary-profile").selectAll("td").data(career.secondaryprofile).text(function(d) {
                return d;
            }), d3.select("#entries").selectAll("li").remove(), d3.select("#entries").selectAll("li").data(career.entries).enter().append("li").append("a").on("click", setActive).text(IDOrName), 
            d3.select("#exits").selectAll("li").remove(), d3.select("#exits").selectAll("li").data(career.exits).enter().append("li").append("a").on("click", setActive).text(IDOrName));
        }
        var d3 = require("d3"), diameter = 1e3, radius = diameter / 2, innerRadius = radius - 180, svg = d3.select("#classwheel").append("g").attr("transform", "translate(" + radius + "," + radius + ")"), bundle = d3.layout.bundle(), link = svg.append("g").selectAll(".link"), node = svg.append("g").selectAll(".node"), cluster = d3.layout.cluster().size([ 360, innerRadius ]), line = d3.svg.line.radial().interpolate("bundle").tension(.15).radius(function(d) {
            return d.y;
        }).angle(function(d) {
            return d.x / 180 * Math.PI;
        }), active_career = void 0, careers = {}, career_list = d3.select("#classlist ul").selectAll("li");
        d3.json("data/careers.json", function(error, data) {
            data.forEach(function(d) {
                careers[d.id] = d;
            }), career_list = career_list.data(Object.keys(careers)), list = career_list.enter().append("li").attr("id", function(d) {
                return "list_" + d;
            }), list.on("click", setActive), list.append("a").text(function(d) {
                return careers[d].name;
            }), list.append("span").attr("class", function(d) {
                return "pull-right label label-" + careers[d].type.toLowerCase();
            }).text(function(d) {
                return careers[d].type[0];
            }), d3.select("#searchform input").on("input", function(d) {
                var text = this.value.toLowerCase();
                text ? career_list.classed("hidden", !1).classed("hidden", function(d) {
                    return careers[d].name.toLowerCase().indexOf(text) < 0;
                }) : career_list.classed("hidden", !1);
            });
            var nodes = cluster.nodes(function() {
                ...
                return nodes.forEach(function(d) {
                    ...
                    c = d.id && careers[d.id], c && (c.entries.forEach(append_links), c.exits.forEach(append_links));
            ...
            }).attr("class", "link").attr("d", line), node = node.data(nodes.filter(function(n) {
                return !n.children;
